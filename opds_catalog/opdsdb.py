...
##########################################################################
# объект который мы будем использовать для перекодироки 4х байтного UTF
# в 3х байтный пока только для аннотации, т.к. там уже "словлена" ошибка
# при записи в 3х байтный utf8 MYSQL
#
...
def arc_skip(arcpath: str, arcsize: int) -> int:
    ...
    и возвращаем 0
    Если книги из искомого каталога имелись и для них установлен avail=2,
    ...
    # Если каталог в БД найден и его размер совпадает с текущим, то считаем
    # что файл архива не менялся. Поэтому делаем update всех книг из этого
    # архива, однако если ни одного изменения не произошло, то таких книг нет,
    # поэтому видимо нужно пересканировать архив
    if arcsize == catalog.cat_size:
        row_count = Book.objects.filter(path=arcpath).update(avail=2)
        return row_count

    # Здесь мы оказываемся если размеры архива в БД и в наличии разные, поэтому
...
def addcattree(cat_name: str, archive: int = 0, size: int = 0) -> Catalog:
    ...
    new_cat = Catalog.objects.create(
        parent=parent,
        cat_name=tail[:SIZE_CAT_CATNAME],
        path=cat_name[:SIZE_CAT_PATH],
        cat_type=archive,
        cat_size=size,
    )
